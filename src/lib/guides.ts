...
export function getGuideHierarchy(
  ...
  if (parentGuideId) {
    ...
    const parentPlanned: PlannedChapter[] = (parentPost?.data.plannedChapters ??
      []) as PlannedChapter[]
  ...
  const childChapters = allLocalePosts
    .filter(
      (p) =>
        (p.data.parentGuide === guideIdentifier || p.data.parentGuide === currentSlug) &&
        !p.data.isPlanned,
    )
    .sort((a, b) => (a.data.order ?? 0) - (b.data.order ?? 0))
...
export function getCoverImageSrc(
  ...
  return typeof coverImage === 'string' && coverImage.trim().length > 0 ? coverImage : fallback
}
