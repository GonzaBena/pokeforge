...
export function typeBadgesHtml(types: string[], small = false): string {
  ...
  return types
    .map(
      (t) =>
        `<span class="${cls}" data-type="${t}" style="--badge-bg:${typeColor(t)}">${getTypeName(t, locale)}</span>`,
    )
    .join('')
}
...
export function formatGeneration(gen: unknown): string {
  return String(gen ?? '')
    .replace(/^(Generación|Generation)\s*/i, '')
    .trim()
}

export function getDefaultAbility(
  detail?: { abilities?: { name: string; isHidden: boolean }[] } | null,
): string | null {
