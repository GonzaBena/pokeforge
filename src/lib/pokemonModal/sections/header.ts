...
import { capitalize, dexNumber, getDefaultAbility, typeBadgesHtml } from '../utils'
...
export function renderHeader(ctx: RenderContext): string {
  ...
  const hasEvolution = Boolean(ctx.chain?.nodes.some((n) => n.evolvesFromSpecies === pokemon.name))
  ...
  const selectedAbility = abilitiesList.find((a) => a.name === currentAbilityName) ?? null
  ...
  const abilityOptionsHtml = abilitiesList
    .map((a) => {
      ...
      const label =
        locale === 'es' ? a.nameEs || capitalize(a.name) : a.nameEn || capitalize(a.name)
...
export function updateHeaderCapturedState(captured: boolean): void {
  ...
  const itemSelect = bodyEl.querySelector<HTMLSelectElement>('[data-item-select]')
