...
export function renderAbilitiesContent(
  ...
  const cardsHtml = abilities
    .map((ab) => {
      const name =
        locale === 'es' ? ab.nameEs || capitalize(ab.name) : ab.nameEn || capitalize(ab.name)
      const desc =
        locale === 'es'
          ? ab.descriptionEs || ab.descriptionEn || ''
          : ab.descriptionEn || ab.descriptionEs || ''
      const isActive = activeAbility === ab.name

      const pct = Math.round(ab.probability * 100)
      const badgeHtml = ab.isHidden
        ? `<span class="detail-ability-card__badge detail-ability-card__badge--hidden"><i data-lucide="eye-off"></i> ${t.modal.hiddenAbility}</span>`
        : `<span class="detail-ability-card__badge detail-ability-card__badge--prob" title="${t.modal.probability}: ${pct}%">${pct}%</span>`
