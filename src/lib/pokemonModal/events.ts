...
export function bindModalEvents(): void {
  ...
  document.addEventListener('input', (e) => {
    ...
    if (
      !target.matches('[data-stat-input]') ||
      modalState.currentId === null ||
      !modalState.lastContext
    )
      return
    ...
    if (modalState.currentSlotIndex !== null) {
      ...
      if (slot) {
        const nextStats = {
          ...modalState.lastContext.detail.stats,
          ...(slot.stats ?? {}),
          [key]: value,
        }
  ...
  document.addEventListener('change', (e) => {
    ...
    if (
      target.matches('[data-link-pokedex]') &&
      ...
      if (isChecked) {
        ...
        const hasPokedexData =
          pOverrides.nature !== null ||
          pOverrides.ability !== null ||
          Object.keys(pOverrides.stats).length > 0
        if (hasPokedexData) {
    ...
    if (
      target.matches('[data-ability-select]') &&
      modalState.currentId !== null &&
      modalState.lastContext
    ) {
      ...
      if (bodyEl && modalState.lastContext) {
        const desc = ability
          ? locale === 'es'
            ? ability.descriptionEs || ability.descriptionEn || ''
            : ability.descriptionEn || ability.descriptionEs || ''
          : ''
        ...
        if (titleEl) {
          titleEl.textContent = ability
            ? locale === 'es'
              ? ability.nameEs || capitalize(ability.name)
              : ability.nameEn || capitalize(ability.name)
            : locale === 'es'
              ? 'Habilidad'
              : 'Ability'
        }
        if (textEl) {
          textEl.textContent =
            desc ||
            (locale === 'es'
              ? 'La habilidad otorga efectos pasivos únicos en combate o aventura.'
              : 'Abilities provide unique passive effects in battle or adventure.')
        }
        if (subEl) {
          subEl.textContent = ability?.isHidden
            ? locale === 'es'
              ? 'Habilidad Oculta'
              : 'Hidden Ability'
            : ''
        ...
        const abilitySection = bodyEl.querySelector<HTMLElement>(
          '[data-section-id="abilities"] .detail-section__content',
        )
        if (abilitySection) {
    ...
    if (target.matches('[data-nature-select]') && modalState.currentId !== null) {
      ...
      if (bodyEl && modalState.lastContext) {
        ...
        if (natureTitleEl) {
          natureTitleEl.textContent = nature
            ? getNatureName(nature.name, locale)
            : locale === 'es'
              ? 'Naturaleza'
              : 'Nature'
        }
        ...
        STAT_KEYS.forEach((key) => {
          const inputEl = bodyEl.querySelector<HTMLInputElement>(
            `[data-stat-input][data-stat-key="${key}"]`,
          )
          ...
          if (statEl) {
            ...
            const labelGroup = statEl.querySelector<HTMLElement>('.detail-stat__label-group')
    ...
    if (
      target.matches('[data-item-select]') &&
      modalState.currentId !== null &&
      modalState.lastContext
    ) {
      const value = (target as HTMLSelectElement).value || null
      ...
      const { bodyEl } = getModalElements()
      if (bodyEl && modalState.lastContext) {
        const effSection = bodyEl.querySelector<HTMLElement>(
          '[data-section-id="effectiveness"] .detail-section__content',
        )
        if (effSection) {
          effSection.innerHTML = renderEffectivenessContent(
            modalState.lastContext.pokemon,
            modalState.lastContext.typeChart,
          )
          refreshIcons()
        }
      }
  ...
  document.addEventListener('click', (e) => {
    ...
    if (toggleBtn) {
      ...
      const locale = getCurrentLocale()
      if (labelEl) {
        labelEl.textContent = modalState.showHexagonChart
          ? locale === 'es'
            ? 'Ocultar Juez'
            : 'Hide Judge'
          : locale === 'es'
    ...
    if (
      ...
      const pOverrides = getPokemonOverrides(modalState.currentId)
      const hasData =
        pOverrides.nature !== null || (pOverrides.stats && Object.keys(pOverrides.stats).length > 0)
      const locale = getCurrentLocale()
    ...
    if (captureBtn && modalState.currentId !== null && modalState.lastContext) {
      ...
      if (nowCaptured) {
        ...
        const selectedAbilityVal =
          abilitySelect?.value || getDefaultAbility(modalState.lastContext.detail)
        ...
        if (abilitySection) {
          abilitySection.innerHTML = renderAbilitiesContent(
            modalState.lastContext.detail,
            currentOverrides.ability ?? selectedAbilityVal,
          )
          refreshIcons()
        }
        ...
        const effectsContainer = bodyEl.querySelector<HTMLElement>(
          '[data-nature-effects-container]',
        )
        if (effectsContainer) {
        ...
        const abilityTooltipDesc = bodyEl.querySelector<HTMLElement>(
          '[data-ability-help-popover] .detail-help-tooltip__desc',
        )
        if (abilityTooltipDesc) {
          ...
          if (matchedAbility) {
            ...
            abilityTooltipDesc.textContent =
              flavor ||
        ...
        toast.info(
          locale === 'es'
            ? `${name} liberado de tu Pokédex.`
            : `${name} released from your Pokédex.`,
        )
    ...
    if (tooltipTrigger) {
      ...
      document
        .querySelectorAll('.detail-help-tooltip.is-open')
    ...
    if (
      tocMenu &&
      tocMenu.classList.contains('is-open') &&
      !target.closest('[data-detail-fab-container]')
    ) {
      closeTocMenu()
    }
  })
  ...
  document.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'Escape') {
      ...
      if (
        target &&
        target.matches('[data-move-search-input]') &&
