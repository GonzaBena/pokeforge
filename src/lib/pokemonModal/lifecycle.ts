...
export async function openPokemonModal(id: number, options?: PokemonModalOptions): Promise<void> {
  ...
  try {
    ...
    const chain =
      detail.evolutionChainId !== null
        ? await getEvolutionChain(detail.evolutionChainId).catch(() => null)
        : null
    if (modalState.currentId !== id) return
    ...
    if (detail.isOfflineFallback) {
      toast.info(
        locale === 'es'
          ? 'Modo sin conexión: mostrando datos básicos disponibles'
          : 'Offline mode: showing basic available data',
      )
    }
  } catch (err) {
