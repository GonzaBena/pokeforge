  ...
  while ((match = tokenRegex.exec(query)) !== null) {
    ...
    if (colonIndex > 0) {
      ...
      if (TYPE_PREFIXES.has(rawPrefix)) {
        const parts = valuePart.split(",").map((v) => normalizeFilterText(v)).filter(Boolean);
...
function matchMethod(actualMethod: string, targetMethod: string): boolean {
  ...
  if (targetMethod === "machine") return actualMethod === "machine" || actualMethod === "tm" || actualMethod === "hm";
