...
describe("Move Filters - parseMoveQuery", () => {
  ...
  it("parses numeric filters for power, pp, accuracy, and level with various operators", () => {
    ...
    assert.deepEqual(exact.numericFilters.find((f) => f.field === "power"), {
      field: "power",
      operator: "=",
      value: 10,
    });
    assert.deepEqual(exact.numericFilters.find((f) => f.field === "pp"), {
      field: "pp",
      operator: "=",
      value: 10,
    });
    assert.deepEqual(exact.numericFilters.find((f) => f.field === "accuracy"), {
      field: "accuracy",
      operator: "=",
      value: 100,
    });
    assert.deepEqual(exact.numericFilters.find((f) => f.field === "level"), {
      field: "level",
      operator: "=",
      value: 25,
    });
    ...
    assert.deepEqual(gt.numericFilters.find((f) => f.field === "power"), {
      field: "power",
      operator: ">",
      value: 30,
    });
    assert.deepEqual(gt.numericFilters.find((f) => f.field === "pp"), {
      field: "pp",
      operator: ">=",
      value: 30,
    });
    assert.deepEqual(gt.numericFilters.find((f) => f.field === "accuracy"), {
      field: "accuracy",
      operator: ">",
      value: 80,
    });
    assert.deepEqual(gt.numericFilters.find((f) => f.field === "level"), {
      field: "level",
      operator: "<=",
      value: 50,
    });
