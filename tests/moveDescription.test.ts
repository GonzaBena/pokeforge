...
import type { MoveTableRow } from '../src/lib/pokemonModal/types'
...
describe('Move Table Row and Column Rendering', () => {
  const sampleDetails: MoveDetail[] = [{ name: 'thunderbolt', method: 'machine', level: 0 }]
  ...
  it("getMoveColumns renders '?' button to the left of the move name with tooltip description", () => {
    ...
    const mockContext = {
      ...
      row: { original: rows[0] },
    } as unknown as CellContext<MoveTableRow, unknown>
    ...
    assert.ok(
      cellHtml.includes('move-help-btn') || cellHtml.includes('move-help'),
      'Cell must have help button class',
    )
