...
describe('Items Database - Integrity & Coverage', () => {
  it('contains no duplicate item IDs and exposes all items', () => {
    ...
    for (const item of all) {
      assert.equal(ids.has(item.id), false, `Duplicate item ID found: ${item.id}`)
      ids.add(item.id)
    }
  })
  ...
  it('ensures all items have valid categories and localized text', () => {
    for (const item of ITEMS_DATABASE) {
      ...
      assert.ok(
        item.shortDescEs && item.shortDescEs.trim().length > 0,
        `Item ${item.id} missing shortDescEs`,
      )
      assert.ok(
        item.shortDescEn && item.shortDescEn.trim().length > 0,
        `Item ${item.id} missing shortDescEn`,
      )
    }
  })
