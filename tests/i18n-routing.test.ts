...
describe('i18n Routing - getLocalizedPath with localized team routes', () => {
  ...
  it('preserves other routes correctly', () => {
    ...
    assert.equal(getLocalizedPath('/dashboard/', 'en'), '/dashboard/')
    assert.equal(getLocalizedPath('/dashboard', 'en'), '/dashboard/')
    assert.equal(getLocalizedPath('/dashboard/', 'es'), '/es/dashboard/')
    assert.equal(getLocalizedPath('/dashboard', 'es'), '/es/dashboard/')
    assert.equal(getLocalizedPath('/es/dashboard/', 'en'), '/dashboard/')
    assert.equal(getLocalizedPath('/dashboard/settings/', 'en'), '/dashboard/settings/')
    assert.equal(getLocalizedPath('/dashboard/settings', 'en'), '/dashboard/settings/')
    assert.equal(getLocalizedPath('/dashboard/settings/', 'es'), '/es/dashboard/settings/')
    assert.equal(getLocalizedPath('/dashboard/settings', 'es'), '/es/dashboard/settings/')
    assert.equal(getLocalizedPath('/es/dashboard/settings/', 'en'), '/dashboard/settings/')
  })
})
