...
async function fetchItemEs(name: string, url: string, force: boolean): Promise<string | null> {
  try {
    const item = await cachedFetch<ItemResponse>(
      'item',
      name,
      () => fetchJson<ItemResponse>(url),
      force,
    )
...
export async function buildEvolutionChains(
  chainIds: number[],
  force = false,
): Promise<BuildEvolutionChainsResult> {
  ...
  await Promise.all(
    chainIds.map((id) => limit(() => buildOneChain(id, force, chains, itemEs, failed))),
  )
