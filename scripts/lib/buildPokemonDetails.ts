...
const OUT_DIR = path.resolve(import.meta.dirname, '..', '..', 'public', 'data', 'pokemon-detail')
...
function extractStats(raw: RawPokemonDetail): PokemonStats {
  const find = (name: string) => raw.stats.find((s) => s.stat.name === name)?.base_stat ?? 0
  return {
    hp: find('hp'),
    attack: find('attack'),
...
function extractAbilities(raw: RawPokemonDetail, abilitiesMap: AbilitiesMap): PokemonAbilityInfo[] {
  ...
  return rawList
    ...
    .map((entry) => {
      ...
      const probability = isHidden
        ? 0
        : nonHiddenCount > 0
          ? Number((1 / nonHiddenCount).toFixed(2))
          : 0
...
function extractMoveDetails(raw: RawPokemonDetail): MoveDetail[] {
  ...
  for (const m of raw.moves) {
    ...
    for (const vgd of m.version_group_details) {
      ...
      const method = rawMethod === 'train' ? 'tutor' : rawMethod
...
function evolutionMethodText(node: EvolutionNode, itemEs: Map<string, string>): string {
  const from = node.evolvesFromSpecies ? formatLabel(node.evolvesFromSpecies) : ''
  const parts: string[] = []
  if (node.minLevel) parts.push(`Nivel ${node.minLevel}`)
  if (node.item) parts.push(`usando ${itemEs.get(node.item) ?? formatLabel(node.item)}`)
  if (!node.minLevel && !node.item && node.trigger && node.trigger !== 'level-up') {
...
function sortAndFormatRows(
  ...
  return [...rows]
    .sort((a, b) => {
      const genDiff =
        (generationOrder.get(a.generation) ?? 0) - (generationOrder.get(b.generation) ?? 0)
      if (genDiff !== 0) return genDiff
      return (
        (versionGroupOrder.get(a.versionGroup) ?? 0) - (versionGroupOrder.get(b.versionGroup) ?? 0)
      )
    })
    .map((row) => ({
...
async function buildOneDetail(
  ...
  try {
    ...
    const wildRows = buildWildRows(encounters, ctx.versionToGroup, ctx.groupToGeneration)
    ...
    if (speciesInfo.evolutionChainId !== null) {
      ...
      evolutionRows = buildEvolutionRows(node, ownGeneration, ctx.generationsByName, ctx.itemEs)
    }
    ...
    await writeFile(path.join(OUT_DIR, `${id}.json`), JSON.stringify(detail), 'utf-8')
  } catch (err) {
...
export async function buildPokemonDetails(
  ...
  const versionGroupDisplay = new Map(
    generations.flatMap((g) => g.versionGroups.map((vg) => [vg.name, vg.displayName] as const)),
  )
  const generationOrder = new Map(generations.map((g) => [g.name, g.id]))
  const versionGroupOrder = new Map(
    generations.flatMap((g) => g.versionGroups.map((vg, i) => [vg.name, i] as const)),
  )
