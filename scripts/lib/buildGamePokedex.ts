...
const VERSION_METADATA: Record<string, { name: string; nameEs: string; color: string }> = {
  ...
  'the-isle-of-armor-sword': {
    name: 'Isle of Armor (Sword)',
    nameEs: 'Isla de la Armadura (Espada)',
    color: '#00a8ff',
  },
  'the-isle-of-armor-shield': {
    name: 'Isle of Armor (Shield)',
    nameEs: 'Isla de la Armadura (Escudo)',
    color: '#e84118',
  },
  'the-crown-tundra-sword': {
    name: 'Crown Tundra (Sword)',
    nameEs: 'Nieves de la Corona (Espada)',
    color: '#00a8ff',
  },
  'the-crown-tundra-shield': {
    name: 'Crown Tundra (Shield)',
    nameEs: 'Nieves de la Corona (Escudo)',
    color: '#e84118',
  },
  'brilliant-diamond': {
    name: 'Brilliant Diamond',
    nameEs: 'Diamante Brillante',
    color: '#0abde3',
  },
  ...
  'the-teal-mask-scarlet': {
    name: 'Teal Mask (Scarlet)',
    nameEs: 'Máscara Turquesa (Escarlata)',
    color: '#eb2f06',
  },
  'the-teal-mask-violet': {
    name: 'Teal Mask (Violet)',
    nameEs: 'Máscara Turquesa (Púrpura)',
    color: '#8854d0',
  },
  'the-indigo-disk-scarlet': {
    name: 'Indigo Disk (Scarlet)',
    nameEs: 'Disco Índigo (Escarlata)',
    color: '#eb2f06',
  },
  'the-indigo-disk-violet': {
    name: 'Indigo Disk (Violet)',
    nameEs: 'Disco Índigo (Púrpura)',
    color: '#8854d0',
  },
...
const CURATED_EXCLUSIVES: Record<string, Record<string, number[]>> = {
  'scarlet-violet': {
    scarlet: [
      246, 247, 248, 425, 426, 434, 435, 633, 634, 635, 690, 691, 765, 874, 936, 984, 985, 986, 987,
      988, 989, 1005, 1007, 1009, 1020, 1021,
    ],
    violet: [
      200, 429, 316, 317, 371, 372, 373, 692, 693, 766, 875, 885, 886, 887, 937, 990, 991, 992, 993,
      994, 995, 1006, 1008, 1010, 1022, 1023,
    ],
  },
  'x-y': {
...
export async function buildGamePokedex(): Promise<GameDexData> {
    ...
    for (const vg of g.versionGroups) {
      ...
      if (versionIds.length === 2) {
        ...
        if (vgName in CURATED_EXCLUSIVES) {
          ...
          if (cur[vA])
            exclusives[vA] = cur[vA].filter((sid) => obtainableSet.has(sid)).sort((a, b) => a - b)
          if (cur[vB])
            exclusives[vB] = cur[vB].filter((sid) => obtainableSet.has(sid)).sort((a, b) => a - b)
          ...
          exclusives[vA] = Array.from(dEnc)
            .filter((sid) => !pEnc.has(sid) && obtainableSet.has(sid))
            .sort((a, b) => a - b)
          exclusives[vB] = Array.from(pEnc)
            .filter((sid) => !dEnc.has(sid) && obtainableSet.has(sid))
            .sort((a, b) => a - b)
        } else {
          ...
          const exA = Array.from(vASet)
            .filter((sid) => !vBSet.has(sid) && obtainableSet.has(sid))
            .sort((a, b) => a - b)
          const exB = Array.from(vBSet)
            .filter((sid) => !vASet.has(sid) && obtainableSet.has(sid))
            .sort((a, b) => a - b)
  ...
  console.log(
    `game-pokedex: generated regional, obtainable, and exclusives for ${Object.keys(result).length} version groups`,
  )
  return result
}
